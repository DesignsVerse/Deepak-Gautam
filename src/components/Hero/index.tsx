...
const Hero = () => {
  ...
  return (
    <section className="relative w-full h-screen md:h-[547px] flex items-center justify-center px-4 sm:px-6 md:px-12 lg:px-16 py-8 bg-gradient-to-b from-[#FDF7F4] to-transparent overflow-hidden">
      ...
      {/* White Container */}
      <motion.div
        className="relative mt-14 z-10 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10 bg-white/85 backdrop-blur-lg max-w-6xl w-full rounded-2xl shadow-2xl overflow-hidden md:h-[400px]"
        initial="hidden"
        animate="visible"
        variants={containerVariants}
      >
        {/* Mobile: Image on Top, Desktop: Image on Right */}
        <div className="relative flex justify-center items-center w-full md:w-1/2 py-4 md:py-0 order-1 md:order-2">
          <motion.div
        ...
        {/* Mobile: Text Centered Below Image, Desktop: Text on Left */}
        <div className="flex-1 flex flex-col justify-center items-center md:items-start text-center md:text-left px-4 sm:px-6 py-4 md:py-6 md:pl-8 lg:pl-10 order-2 md:order-1">
          <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-extrabold text-black leading-tight mb-2 md:mb-3">
            Solve Kaal Sarp & Mangal Dosh with Pandit Ji
          </h1>
          <p className="text-gray-700 text-sm sm:text-base md:text-lg leading-relaxed max-w-xs sm:max-w-sm md:max-w-md hidden md:block">
            Get expert horoscope & Vastu guidance to overcome challenges and achieve peace and prosperity.
          </p>
          <p className="text-gray-700 text-xs sm:text-sm leading-relaxed max-w-xs md:hidden">
            Expert astrology & Vastu solutions for a better future.
          </p>
          <motion.button
            className="mt-3 md:mt-4 px-4 sm:px-6 py-2 bg-gradient-to-r from-[#b91c1c] to-[#7f1d1d] text-white font-semibold rounded-lg shadow-md text-sm sm:text-base"
