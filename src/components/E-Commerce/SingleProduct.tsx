...
const SingleProduct = ({ id }: { id: string }) => {
  ...
  if (!product) {
    return <p className="text-red-500 text-center animate-pulse font-medium">Product not found!</p>;
  }

  const slugifiedName = product.name.toLowerCase().replace(/\s+/g, "-");
  console.log("Product ID:", id, "Name:", product.name, "Slug:", slugifiedName); // Debug slug
  ...
  return (
    <div className="max-w-xs mx-auto bg-white rounded-lg shadow-md overflow-hidden transition-transform hover:scale-105 relative">
      <div className="relative">
        <Link href={`/E-commerce/${slugifiedName}`} className="block" aria-label={`View details for ${product.name}`}>
          <Image
            src={product.image}
            alt={product.name}
            width={220}
      ...
      <div className="p-4 text-center">
        <h3 className="text-sm md:text-base font-semibold text-gray-800 line-clamp-2">{product.name}</h3>
